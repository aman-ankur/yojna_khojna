...
*   **Chunking:** Custom semantic chunking strategy implemented in `document_chunker.py`. Uses `langchain.text_splitter.RecursiveCharacterTextSplitter` internally but adds pre/post-processing for semantic boundaries (sections, tables).
*   **Other:** `python-dotenv` (config), `pdfplumber` (PDF text/table extraction), `pytesseract` (OCR fallback), `spacy` (NER for enhanced retrieval)
...
*   **Problem Addressed:** Default chunking (e.g., by character count) often splits semantically related content like tables or sections, harming retrieval quality.
*   **Implementation (`document_chunker.py`):**
    *   **Pre-processing:**
        *   Identifies complete table blocks (using `[TABLE_START:...]` and `[TABLE_END]` markers from `pdf_extractor.py`) via regex and replaces them with unique placeholders (e.g., `__TABLE_uuid__`).
        *   Identifies section headers (English: "Section X.Y", Hindi: "खंड X.Y") using regex.
        *   Inserts a special `__SECTION_BREAK__` marker immediately before each detected section header in the placeholder-modified text.
    *   **Chunking:**
        *   Uses `langchain.text_splitter.RecursiveCharacterTextSplitter`.
        *   Configured with `SEPARATORS = ["__SECTION_BREAK__", "\n\n", "\n", ". ", ...]` to prioritize splitting at section markers, then paragraphs, etc.
        *   Operates on the pre-processed text containing placeholders and section markers.
    *   **Post-processing:**
        *   Iterates through the raw chunks generated by the splitter.
        *   Replaces table placeholders (`__TABLE_uuid__`) with their original, full table content.
        *   Removes the `__SECTION_BREAK__` markers.
        *   Strips leading/trailing whitespace.
*   **Benefits:** Aims to keep tables intact within chunks and encourages chunks to align with document sections, providing more contextually relevant segments for the RAG system.
*   **Dependencies:** Relies on the table markers inserted by the enhanced `pdf_extractor.py`.
